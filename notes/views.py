    ...
    if request.method == "POST":
        ...
        try:
            Notes.objects.create(
                user=u, uploadingdate=date.today(), branch=b, subject=s, notesfile=n, filetype=f, description=d, status='pending')
            error = "no"
        except:
            error = "yes"
    d = {'error': error}
...
def delete_mynotes(request, pid):
    ...
    notes.delete()
    return redirect('view_mynotes')  # or 'views_notes.hmtl'
...
def delete_users(request, pid):
    ...
    users.delete()
    return redirect('view_users')  # or 'views_notes.hmtl'
